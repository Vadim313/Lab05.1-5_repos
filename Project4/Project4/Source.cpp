...
double S0(const int K, const int N)
{
	double s = 0;
	for (int i = K; i <= N; i++)
		s += sin(1. * K) * sin(1. * K) + cos(1. * N / K) * cos(1. * N / K);
	return s;
}
...
double S1(const int K, const int N, const int i)
{
	if (i > 19)
		...
		return sin(1. * K) * sin(1. * K) + cos(1. * N/K) * cos(1. * N / K) + S1(K, N, i + 1);
}
...
double S2(const int K, const int N, const int i)
{
	if (i < K)
		...
		return sin(1. * K) * sin(1. * K) + cos(1. * N / K) * cos(1. * N / K) + S2(K, N, i - 1);
}

double S3(const int K, const int N, const int i, double t)
{
	t = t + sin(1. * K) * sin(1. * K) + cos(1. * N / K) * cos(1. * N / K);
	if (i >= 19)
		return t;
	else
		return S3(K, N, i + 1, t);
